...
def runEventExtract(text = None):
    ...
    return engine.rule_library.result[0], newsList
...
def runDatabase(d1, d2,c1 = None):
    ...
    return engine.rule_library.Company1
...
def runManualInput( detail, trend ,item = None, business = None, index = None, country = None , d1 = datetime.now(), d2 = datetime.now() + timedelta(days=2)):
    ...
    for number, comp in enumerate(allCompany.companyList[0:]):
        ...
        eng.reset(Company1 = engine.rule_library.Company1 ,Date_Begin = beginDate, Date_End = endDate, manualInput= m)
        eng.run()
        engine.rule_library.result[-1].addResult(engine.rule_library.Company1,'结束')
        engine.rule_library.allProduct = []
        engine.rule_library.allBusiness = []
    ...
    return engine.rule_library.result[-1]
...
if __name__ == "__main__":
    ...
    # runDatabase(datetime(2020, 3, 27, 0, 0),datetime(2020, 8, 29, 0, 0))
    # runDatabase(datetime(2020, 3, 27, 0, 0),datetime(2020, 8, 29, 0, 0),'601857SH')
    # runDatabase(datetime(2020, 3, 27, 0, 0),datetime(2020, 8, 29, 0, 0),'601898SH')
    # runDatabase(datetime(2020, 3, 27, 0, 0),datetime(2020, 8, 29, 0, 0),'600121SH')
