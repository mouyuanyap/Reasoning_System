...
from concept.company import Company
from concept.country import Country
from concept.futures import Futures
from concept.industry import Industry
from concept.person import Person
...
class Term(object):
    ...
    def __init__(self, operator='null', variables='null', comments='', *args, **kwargs):
        ...
        if not operator == 'null' and not operator == None:
            ...
            for pos, (variable, v_type) in enumerate(zip(self.variables, self.operator.inputType)):
                ...
                if type(variable) in [str, int, float, list, tuple, dict, Country, datetime, Company, Futures,Industry,Person]:
    ...
    def GetRHS(self):
        ...
        if type(RHS) in [str, int, float, list, tuple, dict]:
            #print('{},{}'.format(self.operator,self.variables))
